import type { Task, Dependency, LogEntry, WatcherState, DebussyConfig, UiPrefs } from '../../shared/types'
...
interface DebussyAPI {
  tasks: {
    ...
    advanceTo: (id: string, toStage: string)       => Promise<{ success: boolean }>
  ...
  conductor: {
    ...
    getSessionId:    ()                                                       => Promise<{ sessionId: string | null; contextSummary: string | null; historySummary: string | null }>
  ...
  uiPrefs: {
    get: () => Promise<UiPrefs>
    set: (key: keyof UiPrefs, value: boolean) => Promise<{ success: boolean; error?: string }>
  }
}
